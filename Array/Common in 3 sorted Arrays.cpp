...
vector<int> commonElements(vector<int> &arr1, vector<int> &arr2,
    ...
    */
    ...
    //O(m+n+p)  array traversing only once
    vector<int> result;
    int i=0, j=0, k=0;
    while(i<arr1.size() && j<arr2.size() && k<arr3.size()){
        if(arr1[i]==arr2[j] && arr2[j]==arr3[k]){
            ...
            //handling duplicates
            while(i+1 < arr1.size() && arr1[i]==arr1[i+1]) i++; // Safe memory access, no crash, i+1 in i+1 < arr1.size()
            while(j+1 < arr2.size() && arr2[j]==arr2[j+1]) j++;
            while(k+1 < arr3.size() && arr3[k]==arr3[k+1]) k++;
